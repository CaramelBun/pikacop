...
exports.execute = async (client, message, args) => {
    try {
        ...
        if (!args[0]) {
            ...
            } else if (requiredHelp == 'level' || requiredHelp == 'lvl') {
                let mod = ''

                if (message.member.permissions.has('MANAGE_GUILD')) {
                    mod = '\n\nModerator part coming soon'
                }

                let help = new MessageEmbed()
                .setTitle("**__LEVEL__**")
                .setDescription("<a:blueright:923826794610704384> **What is a level command?**\nA level command is used to check your current level and the level of your friends!\n\n<a:blueright:923826794610704384> **How do I use the level command?**\nYou can easily check your current level and the level of your friends by running the following command:\n\n> `" + db.get(`${message.guild.id}.prefix`) + "level [user]`\n\nExample: \n\n> `" + db.get(`${message.guild.id}.prefix`) + "level " + client.user.tag + "`\n\n<a:blueright:923826794610704384> Important things to keep in mind when running the level command:\n<:continueReply:941030243295170620> The `[user]` part of the command is an optional argument is **optional** and **not required** if not needed!\n<:endReply:941030019428397058> You can check your friends' levels by replacing the `[user]` part of the command with one of the following: \n     <a:blueright:923826794610704384> **User Mention**\n     <a:blueright:923826794610704384> **User ID** (`18` digits)\n     <a:blueright:923826794610704384> **User Tag** (`" + client.user.tag + "`)" + mod)
                .addFields(
                    { 
                        name: '\u200B', 
                        value: '**__Stats__**' 
                    ...
                    {
                        name: 'Base Command:',
                        value: "`"+ db.get(`${message.guild.id}.prefix`) +"level [user]`",
                        inline: false
                    },
                    {
                        name: 'Aliases:',
        ...
        error(message, err, 'HELP_GENERAL_ERR')
    }
}
